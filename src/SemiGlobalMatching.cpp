...
bool SemiGlobalMatching::Initialize(const sint32& width, const sint32& height, const SGMOption& option)
{
    ...
    if (option.census_size == Census5x5) {  // this one
...
{
	// 左右影像census变换
    if (option_.census_size == Census5x5) {  // this one
...
void SemiGlobalMatching::CostAggregation() const
{
    ...
    for (sint32 i = 0; i < size; i++) {
        if (option_.num_paths == 4 || option_.num_paths == 8) {
            cost_aggr_[i] = static_cast<uint16>(cost_aggr_1_[i]) + static_cast<uint16>(cost_aggr_2_[i])
                          + static_cast<uint16>(cost_aggr_3_[i]) + static_cast<uint16>(cost_aggr_4_[i]);
        }
        if (option_.num_paths == 8) {
            cost_aggr_[i] += static_cast<uint16>(cost_aggr_5_[i]) + static_cast<uint16>(cost_aggr_6_[i])
                           + static_cast<uint16>(cost_aggr_7_[i]) + static_cast<uint16>(cost_aggr_8_[i]);
        }
    }
}
...
void SemiGlobalMatching::ComputeDisparity() const
{
    ...
    const bool is_check_unique = option_.is_check_unique;       // true
	const float32 uniqueness_ratio = option_.uniqueness_ratio;  // 0.99
	...
	for (sint32 i = 0; i < height; i++) {
        for (sint32 j = 0; j < width; j++) {
            ...
            if (is_check_unique) {
                ...
                // 判断唯一性约束
                // 若(sec-min < min*(1-uniquness)，则为无效估计
...
void SemiGlobalMatching::LRCheck()
{
    ...
    const float32& threshold = option_.lrcheck_thres;  // 1.0f
        ...
        for (sint32 j = 0; j < width; j++) {
        	...
        	if(col_right >= 0 && col_right < width) {
        		...
        		if (abs(disp - disp_r) > threshold) {
					// 区分遮挡区和误匹配区
					// 通过右影像视差算出在左影像的匹配像素，并获取视差disp_l
					// if(disp_l > disp) 这一步感觉有些问题？
        			//		pixel in occlusions
					// else 
        			//		pixel in mismatches
