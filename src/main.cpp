 ...
 * \param eg. ../data/endo/l_0123.png ../data/endo/r_0123.png 0 192
 * \param eg. ../data/endo/ls_0123.png ../data/endo/rs_0123.png 0 64
 * \return
 */
int main(int argv, char** argc)
{
    ...
    if (!sgm.Initialize(width, height, sgm_option)) {
        std::cout << "SGM Initializing failed!" << std::endl;
        return -2;
    }
    ...
    float fx = 1194.072289*0.25;
    float fy = 1194.072289*0.25;
    float cx = 960*0.25;
    float cy = 540*0.25;
    float b_dis = b_dis = 4.05;
    for (sint32 i = 0; i < height; i++) {
        for (sint32 j = 0; j < width; j++) {
            ...
            if (disp != Invalid_Float) {
                ...
                // neal::logger(neal::LOG_INFO, std::to_string(disp));
                /* calculate xyz */
                float Zw = fx * b_dis / disp;
                float Xw = (j+1-cx) * Zw / fx;
                float Yw = (i+1-cy) * Zw / fy;
                if (Zw>5 && Zw<150) {
                    neal::logger(neal::LOG_INFO, std::to_string(Xw)+" "+std::to_string(Yw)+" "+std::to_string(Zw));
                }
            }
        }
    }
